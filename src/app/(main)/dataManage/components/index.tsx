...
import { getStatByTimeRange } from "@/app/actions";
...
export function TimeCard() {
  const { run, data } = useRequest(getStatByTimeRange, { manual: true });
  const getTime = ([d1, d2]: Dayjs[]) => ({
    startTime: d1.format("YYYYMMDD"),
    endTime: d2.format("YYYYMMDD"),
  });
  useEffect(() => {
    run(getTime([dayjs().startOf("week"), dayjs()]));
  }, []);
  return (
    <Card
      title="数据管理"
      extra={
        <DatePicker.RangePicker
          ...
          defaultValue={[dayjs().startOf("week"), dayjs()]}></DatePicker.RangePicker>
      }>
      <div className="grid grid-cols-4">
        <div className="text-center border-r border-solid ">
          <div className="text-base ">活跃数(人)</div>
          <div className="text-lg font-bold mt-4">{data?.data?.activeMembers || 0}</div>
        </div>
        <div className="text-center border-r border-solid ">
          <div className="text-base ">有效活跃数(人)</div>
          <div className="text-lg font-bold mt-4">{data?.data?.upMicMembers || 0}</div>
        </div>
        <div className="text-center border-r border-solid">
          <div className="text-base ">有效收益款(人)</div>
          <div className="text-lg font-bold mt-4">{data?.data?.incomeMembers || 0}</div>
        </div>
        <div className="text-center ">
          <div className="text-base ">总流水(钻石)</div>
          <div className="text-lg font-bold mt-4">{data?.data?.turnover || 0}</div>
        </div>
      </div>
    </Card>
  );
