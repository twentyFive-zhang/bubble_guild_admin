import { ChartAreaAxes } from "@/app/(main)/dashboard/components/chart";
    ...
    <Card>
      <div className="grid grid-cols-5">
        ...
        <div className="text-center border-r border-solid ">
          ...
          <div className="text-lg font-bold mt-4">{data?.sendGiftNewUserCount || 0}</div>
        </div>
        <div className="text-center border-r border-solid">
        ...
        <div className="text-center  border-r border-solid">
          ...
          <div className="text-lg font-bold mt-4">{data?.directTurnover || 0}</div>
        </div>
        <div className="text-center">
          <div className="text-base ">背包流水(钻石)</div>
          <div className="text-lg font-bold mt-4">{data?.packageTurnover || 0}</div>
        </div>
      </div>
      {!isHideChart && (
        <>
          <Divider></Divider>
          <ChartAreaAxes list={list || []} description={description}></ChartAreaAxes>
        </>
      )}
    </Card>
  );
