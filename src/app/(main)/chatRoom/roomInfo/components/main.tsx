...
import { getJoinPage, getRoomList } from "@/app/actions";
...
export default function Main() {
  ...
  return (
    <div>
      <CommonTable
        initialValues={{ timeType: 6, statTime: dayjs().startOf("week").format("YYYYMMDD") }}
          ...
          { title: "新用户", dataIndex: "sendGiftNewUserCount" },
          {
            ...
            render: (_v, record) => (
              <Button
                type="link"
                onClick={() => {
                  router.push(`${pathname}/detail?roomId=${record.roomId}`);
                }}>
                详情
              </Button>
            ),
