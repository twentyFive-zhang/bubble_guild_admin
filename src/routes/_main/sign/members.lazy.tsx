...
import { exportMemberList, getMemberPage, removeMember } from "@/services";
import { membersTabAtom } from "@/store/members";
...
function Main() {
	...
	const [items, setItems] = useAtom(membersTabAtom);
						...
						<Modal
							...
							onOk={() => {
								modalForm.validateFields().then((res) => {
									...
									setItems((pre) =>
										pre.map((item) =>
											item.key === res.key ? { ...item, ...res } : item,
										),
									);
									form.resetFields();
									const {
								...
								<Button
									...
									onClick={() => {
										const { date, signDate, activeDate, ...rest } =
											form.getFieldsValue() as API.PageParams<{
												// 开始时间
...
export const Route = createLazyFileRoute("/_main/sign/members")({
	component: Main,
});
